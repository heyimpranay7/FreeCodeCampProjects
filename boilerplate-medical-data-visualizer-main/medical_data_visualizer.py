...
# Import data
path = 'FreeCodeCamp/boilerplate-medical-data-visualizer-main/medical_examination.csv'
df = pd.read_csv(path)
...
def draw_cat_plot():
    ...
    df_cat = pd_melt(df,id_vars=['cardio'],value_vars = [('cholesterol', 'gluc', 'smoke',   
...
def draw_heat_map():
      # Clean the data
    df_heat = df[(df['height'] >= df['height'].quantile(0.025)) & (df['height'] <=   
    df['height'].quantile(0.975)) & (df['weight'] >= df['weight'].quantile(0.025)) & (df['weight'] 
     <= df['weight'].quantile(0.975)) & (df['ap_lo'] <= df['ap_hi'])]
